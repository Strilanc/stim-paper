...
Noise channels are tested by simulating circuits that apply the noise channel to simple states, and performing statistical tests on the output samples which verify consistency against independent definitions of the same noise channel from Cirq \cite{quantum_ai_team_and_collaborators_2020_4062499}.
The \path{X_ERROR}/\path{Z_ERROR} transposition that I mentioned slipped through these checks because originally this testing only exercised the bulk sampling API (which was a natural choice due to needing many samples), not the single-sample interactive API.
